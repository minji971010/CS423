...
def get_ranks_by_permu(img_list, sim_dict, threshold=0.2, df=0.85, max_loop=100):
    ...
    while cur_loop < max_loop:
        ...
        for i in range(img_num):
            ref_rank_num_list = map(lambda idx: (rank_list[idx], num_list[idx]), ref_list[i])
...
get_ranks_by_permu([1] * 5, sim_permu, max_loop=100)